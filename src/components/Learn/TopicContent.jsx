...
export default function TopicContent() {
  ...
  return (
    <div className="content-wrapper bg-gray-50 w-full pt-8">
      {content.map((data) => (
        <div key={data._id} className="flex flex-col-reverse lg:flex-row">
          <div className="mx-4 mt-16 lg:mt-0 lg:mx-8">
          ...
          <div className="fixed top-24 bg-white lg:top-0 z-0 lg:bg-gray-50 lg:relative w-full flex items-center justify-start lg:block lg:w-2/12 border-b lg:border-0 px-4 pt-4 lg:pt-0 lg:px-8 mb-2 pl-10 lg:pl-0 lg:mb-0">
            <div className="w-full flex overflow-x-scroll scroll-smooth lg:block">
              <TbMenuDeep className="hidden lg:block text-2xl transform scale-x-[-1] mb-4 my-2 lg:my-0" />
              {data.meta.sections.map((section) => (
                <div
                  key={section.id}
                  className="w-fit flex space-x-2 snap-start scroll-ml-2 lg:block lg:space-x-0 lg:scroll-ml-0"
                  onClick={() => scrollToSection(section.href)}
                >
                  <p className="w-fit text-xs font-medium text-blue-200 hover:text-primary cursor-pointer border lg:border-0 p-2 lg:p-0 m-2 lg:m-0 lg:mt-2 rounded-full lg:rounded-none">
                    {section.name}
                  </p>
                </div>
              ))}
